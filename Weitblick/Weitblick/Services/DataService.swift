...
/*
 ...
 DataService:
 ============
    - Collection of static functions. Each function getting specified id or date to access to REST API in order
      to fetch required data and convert them into adequate Object/Objectlists.
    - To change codelindes, make sure to differentiate between NSObject and NSArrays.
    - Each function initializes a completion handler which fires after completion() with a specified object
      which will be sent to the instance who called the static function.
    - To have a better overview of which elements are necessary to fetch, please have a look at REST API wiki of weitblicker.org or at Model initializers in their definition.
 */
...
class DataService{
    ...
    /*
     ...
     GetNewsByID:
     ============
        - Fetching NewsObject Data with specified ID from REST API.
        - If no image exists for this dataset, a default image will be set.
        - News text will be transformed into clean text. HTML tags will be removed as well as Image tags.
        - Function will return Newsobject in Completionhandler.
     */
    ...
    LoadNews:
    =========
        - Fetching NewsObject Datasets with specified Date from REST API.
        - Getting Datasets with startdate as specified Date and later.
        - Size of Newsobjectlist is 5.
        - If no image exists for this dataset, a default image will be set.
        - News text will be transformed into clean text. HTML tags will be removed as well as Image tags.
        - Function will return Newsobjectlist in Completionhandler.
    */
    ...
    static func loadNews(date : Date,completion: @escaping (_ newsList : [NewsEntry]) -> ()){
        ...
        URLSession.shared.dataTask(with: task, completionHandler: {(data,response,error) -> Void in
            if let data = data{
                guard let jsondata = try? JSONSerialization.jsonObject(with: data, options: .allowFragments) else {return}
    ...
    /*
     ...
     GetProjectWithID:
     =================
        - Fetching Projectobject Data with specified ID from REST API.
        - If no image exists for this dataset, a default image will be set.
        - Project text will be transformed into clean text. HTML tags will be removed as well as Image tags.
        - Function will return ProjectObject in Completionhandler.
        - Cellcount: Counting occurences of several data to configurate Projectdetailcell.
     */
    ...
    static func getProjectWithID (id: Int, completion: @escaping (_ project: Project) -> ()){
        ...
        let request = URLSession.shared.dataTask(with: task){(data, response, error) in
            ...
            if let projectDict = jsondata as? NSDictionary{
                ...
                guard let donationGoal = projectDict.value(forKey : "donation_goal") else { return }
                var donationGoalFloat : Float = 0.0
                if let donationGoalNumber = donationGoal as? NSNumber{
                    donationGoalFloat = Float.init(truncating: donationGoalNumber)
                }
                
                guard let donationDescription = projectDict.value(forKey: "description") else { return }
                var donationDescriptionString : String = ""
                if let donationGoalObj = donationDescription as? String{
                    donationDescriptionString = donationGoalObj
                }
                
                guard let donationCurrent = projectDict.value(forKey: "donation_current") else { return }
                var donationCurrentFloat : Float = 0.0
                if let donationCurrentNumber = donationCurrent as? NSNumber{
                    donationCurrentFloat = Float.init(truncating: donationCurrentNumber)
                }
                
                var donationBankAccount : BankAccount = BankAccount()
                guard let donationAccount = projectDict.value(forKey : "donation_account") else { return }
                if let donationAccountDict = donationAccount as? NSDictionary{
                    guard let holder = donationAccountDict.value(forKey: "account_holder") else { return }
                    let holderString = holder as! String
                    guard let iban = donationAccountDict.value(forKey: "iban") else { return }
                    let ibanString = iban as! String
                    guard let bic = donationAccountDict.value(forKey: "bic") else { return }
                    let bicString = bic as! String
...
LoadProjects:
=============
     - Fetching ProjectObject Datasets with specified Date from REST API.
     - Getting Datasets with startdate as specified Date and later.
     - Size of Projectobjectlist is 5.
     - If no image exists for this dataset, a default image will be set.
     - Project text will be transformed into clean text. HTML tags will be removed as well as Image tags.
     - Function will return Projectobjectlist in Completionhandler.
     - Cellcount: Counting occurences of several data to configurate Projectdetailcell.
*/
...
/*
 ...
 DateFunctions:
 ==============
     - Several functions to convert String into Date.
     - Regex functions to determine regex and replace them.
 */
